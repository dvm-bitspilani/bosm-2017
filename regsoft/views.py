...
def view_captain(request, tc_id):
	...
	rows = []
	for p in participant_list:
		rows.append({'data':[str(p.name).title(), p.firewallz_passed, ], 'link':[{'url':'#', 'title':'Abc'}]})
	tables = {
...
def get_captains(request):
	g_ls = GroupLeader.objects.filter(pcr_approved=True)
	rows = [{'data':[g_l.college, count_players(g_l)], 'link':[{'title':'Create Bills', 'url':reverse('regsoft:create_bill', kwargs={'gl_id':g_l.id})}, {'title':'View Bills', 'url':reverse('regsoft:view_bills', kwargs={'gl_id':g_l.id})}]} for g_l in g_ls if count_players(g_l)!=0]
...
def create_bill(request, gl_id):
	...
	return render(request, 'regsoft/create_bill.html', {'g_leader':g_leader, 'participant_list':participant_list,})
...
def bill_details(request, b_id):
	...
	c_rows = [{'data':[part.name, part.captain.name, part.captain.event.name,], 'link':[{'title':'Print Bill', 'url':reverse('regsoft:print_bill',kwargs={'b_id':bill.id})}]} for part in bill.participant_set.all()]
	table = {
		'title' : 'Participant details for the bill under ' + bill.g_leader.name + ' from ' + bill.g_leader.college,
		'headings' : ['Name', 'Captain', 'Event',],
		'rows':c_rows,
	}
...
def print_bill(request, b_id):
	...
	bill = get_object_or_404(id=b_id)
	g_leader = bill.g_leader
	part_list = bil.participant_set.all()
	try:
	...
	return render(request, 'regsoft/print_bill.html', {'part_list':part_list, 'g_leader':g_leader, 'time':time_stamp, 'bill':bill, 'payment_methods':payment_methods, 'total':total, 'number':number})
